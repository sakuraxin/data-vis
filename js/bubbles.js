function bubbleChart() {
  ...
  var tooltip = floatingTooltip('bubbles_tooltip', 50);

  //var lg = legend();

  // Locations to move bubbles towards, depending
  // on which view mode is selected.
  var center = { x: width / 2, y: height / 2 - 40};


  var yearCenters = {
    "t1": { x: width / 3.5, y: height / 2 },
    "t2": { x: width / 2.5, y: height / 2 },
    "t3": { x: 1.5 * width / 3, y: height / 2 },
    "t4": { x: 2 * width / 3, y: height / 2}
  };
  ...
  var fillColor = d3.scaleOrdinal()
      .domain(["t1","t2","t3","t4"])
      .range(["blue", "#dc3912", "#ff9900", "#00a388"]);


  function createNodes(rawData) {

    //var maxAmount = d3.max(rawData, function (d) { return +d.average; });
